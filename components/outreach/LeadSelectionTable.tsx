    ...
    <div className="glass-card p-6 space-y-4">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {leads.map(lead => (
            <Card key={lead._id} className="bg-black/30 border-white/5 relative">
              <CardContent className="p-4 space-y-2">
                <div className="absolute top-3 right-3"><Checkbox checked={selectedIds.includes(lead._id)} onCheckedChange={() => toggleOne(lead._id)} /></div>
                ...
                <div className="flex justify-between items-center mt-2">
                  <span className={`px-2 py-0.5 rounded-full text-xs ${lead.status === "converted" ? "bg-green-500/20 text-green-300" : "bg-gray-500/20 text-gray-400"}`}>{lead.status}</span>
                </div>
              </CardContent>
            </Card>
          ))}
