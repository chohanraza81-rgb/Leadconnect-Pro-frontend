...
import { Sparkles, Send, Phone, Clock, Mail, MessageCircle } from "lucide-react";
...
export default function Sequencer() {
      ...
      const res = await fetch(`${API}/outreach/campaigns`, {
        ...
        body: JSON.stringify({
          ...
          sequence: sequence.map((body, i) => ({
            ...
            subject: `Day ${i + 1} - ${i === 0 ? "Introduction" : i === 1 ? "WhatsApp Follow-up" : "Final Follow-up"}`,
            body,
          })),
        }),
      });
  ...
  const sendDayEmail = async (stepIndex: number) => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: `Day ${stepIndex + 1} email sent! 📧` });
      } else {
        toast({ title: "Failed to send email. Check Gmail settings.", variant: "destructive" });
      }
    } catch (e) {
  ...
  const openWhatsApp = async () => {
    ...
    const msg = encodeURIComponent(sequence[1]?.substring(0, 250) || "Hi, following up on my email!");
    const link = `https://wa.me/${lead.phone.replace(/[^0-9]/g, "")}?text=${msg}`;
    ...
    <div className="glass-card p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Input
            placeholder="e.g. Free 30-min consultation"
            value={offer}
      ...
      >
        ...
        {generating ? "🧠 Generating..." : "Generate 3-Step Sequence"}
      </Button>
      ...
      {sequence.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div key={i} className="p-4 bg-black/30 rounded-xl border border-white/5">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-gray-500 flex items-center gap-1">
                  {i === 1 ? <MessageCircle className="h-3 w-3" /> : <Mail className="h-3 w-3" />}
                  {i === 1 ? "WhatsApp" : "Email"}
                </span>
              </div>
              <Textarea
                ...
                rows={4}
                className="bg-transparent border-white/10 resize-none text-sm"
              />
            </div>
          ))}
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <Button onClick={saveCampaign} variant="outline" className="border-[#6366F1] text-[#6366F1] hover:bg-[#6366F1]/10">
              💾 Save Campaign
            </Button>
          </div>
          ...
          {activeCampaignId && (
            <div className="flex flex-wrap gap-3 pt-2">
              ...
              >
                ...
                {sendingDay === 0 ? "⏳ Sending..." : "Day 1 Email"}
              </Button>
              <Button
              ...
              >
                ...
                {sendingDay === 2 ? "⏳ Sending..." : "Day 7 Follow-up"}
              </Button>
            </div>
          )}
        </motion.div>
