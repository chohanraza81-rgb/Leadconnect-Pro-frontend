...
import { 
  Sparkles, Send, Phone, Clock, Mail, MessageCircle, 
  Save, Trash2, Copy, CheckCircle2, XCircle, Loader2 
} from "lucide-react";
...
export default function Sequencer() {
  ...
  const generateSequence = async () => {
    if (!selectedLeadId) {
      toast({ title: "Please select a lead first", variant: "destructive" });
      return;
    }
    if (!offer.trim()) {
      toast({ title: "Please enter your offer", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (data.emails && data.emails.length >= 3) {
        ...
        toast({ title: "Failed to generate. Check Groq API key.", variant: "destructive" });
      }
    } catch (e) {
      toast({ title: "Error generating sequence. Is backend running?", variant: "destructive" });
    }
    setGenerating(false);
  };
  ...
  const saveCampaign = async () => {
    ...
    if (sequence.length === 0) {
      toast({ title: "Generate a sequence first", variant: "destructive" });
      return;
    }
      ...
      const res = await fetch(`${API}/outreach/campaigns`, {
        ...
        body: JSON.stringify({
          ...
          sequence: sequence.map((body, i) => ({
            ...
            subject: `Day ${i + 1} - ${i === 0 ? "Introduction" : i === 1 ? "WhatsApp Follow-up" : "Final Call to Action"}`,
            body,
          })),
        }),
      });
  ...
  const sendDayEmail = async (stepIndex: number) => {
    if (!activeCampaignId) {
      toast({ title: "Save campaign first!", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: `📧 Day ${stepIndex + 1} email sent to ${lead.email}!` });
      } else {
  ...
  const openWhatsApp = async () => {
    ...
    const msg = encodeURIComponent(sequence[1]?.substring(0, 250) || "Hi, following up on my previous message!");
    ...
    toast({ title: "💬 WhatsApp opened!", description: `Messaging ${cleanPhone}` });
  };
  ...
  return (
    <div className="glass-card p-6 space-y-6">
      ...
      {/* Step 1: Select Lead & Offer */}
      <div className="bg-black/20 rounded-xl p-5 border border-white/5 space-y-4">
        <h3 className="text-sm font-medium text-gray-300 flex items-center gap-2">
          <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[#6366F1] text-white text-xs">1</span>
          Select Lead & Offer
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs text-gray-400">
                {selectedLead.email && (
                  <span className="flex items-center gap-1 bg-white/5 px-2 py-1 rounded">
                    <Mail className="h-3 w-3 text-blue-400" /> {selectedLead.email}
                  </span>
                )}
                {selectedLead.phone && (
                  <span className="flex items-center gap-1 bg-white/5 px-2 py-1 rounded">
                    <Phone className="h-3 w-3 text-green-400" /> {selectedLead.phone}
                  </span>
                )}
              </div>
            )}
          ...
          <div>
            ...
            <Input
              placeholder="e.g. Free 30-min strategy call"
              value={offer}
        ...
        <Button
          ...
          className="bg-[#6366F1] hover:bg-[#4f46e5] gap-2 w-full md:w-auto"
        >
          {generating ? (
            <><Loader2 className="h-4 w-4 animate-spin" /> Generating...</>
          ) : (
            <><Sparkles className="h-4 w-4" /> Generate 3-Step Sequence</>
          )}
        </Button>
      </div>
      ...
      <AnimatePresence>
        ...
        {sequence.length > 0 && !generating && (
          <motion.div
            ...
            className="bg-black/20 rounded-xl p-5 border border-white/5 space-y-4"
          >
            <h3 className="text-sm font-medium text-gray-300 flex items-center gap-2">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[#6366F1] text-white text-xs">2</span>
              Review & Edit Sequence
            </h3>
                  ...
                  <div key={i} className="relative group">
                    ...
                    <div className="p-4 bg-black/30 rounded-xl border border-white/5 ml-3">
                      ...
                      <Textarea
                        ...
                        rows={4}
                        className="bg-transparent border-white/10 resize-none text-sm font-normal"
                      />
                    </div>
                  </div>
                );
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <Button
                ...
                variant="outline"
                className={`gap-2 ${saved ? 'border-green-500 text-green-400' : 'border-[#6366F1] text-[#6366F1] hover:bg-[#6366F1]/10'}`}
              >
      ...
      <AnimatePresence>
        {activeCampaignId && sequence.length > 0 && (
          ...
          >
            <h3 className="text-sm font-medium text-gray-300 flex items-center gap-2">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-green-500 text-white text-xs">3</span>
              Execute Outreach
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <Button
                ...
                disabled={sendingDay === 0 || !selectedLead?.email}
                className="bg-[#6366F1] hover:bg-[#4f46e5] h-auto py-4 flex-col gap-2"
              >
                {sendingDay === 0 ? (
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs opacity-80">Open WhatsApp</div>
                </div>
              </Button>
              ...
              <Button
                ...
                disabled={sendingDay === 2 || !selectedLead?.email}
              ...
              >
                ...
                <div className="text-center">
                  <div className="font-semibold text-sm">Day 7</div>
                  <div className="text-xs opacity-80">Send Follow-up</div>
                </div>
              </Button>
            </div>
            ...
            <div className="flex flex-wrap gap-4 text-xs text-gray-400">
              <span className="flex items-center gap-1">
                {selectedLead?.email ? <CheckCircle2 className="h-3 w-3 text-green-400" /> : <XCircle className="h-3 w-3 text-red-400" />}
                Email: {selectedLead?.email || "Not available"}
              </span>
              <span className="flex items-center gap-1">
                {selectedLead?.phone ? <CheckCircle2 className="h-3 w-3 text-green-400" /> : <XCircle className="h-3 w-3 text-red-400" />}
                Phone: {selectedLead?.phone || "Not available"}
              </span>
            </div>
          </motion.div>
        )}
