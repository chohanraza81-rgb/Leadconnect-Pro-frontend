...
function mergeData(whatsapp: any[], emails: any[]) {
  ...
  (whatsapp || []).forEach(w => { const key = w._id || w.date || ''; if (key) map[key] = { date: key, whatsapp: w.count || 0, emails: 0 }; });
  (emails || []).forEach(e => { const key = e._id || e.date || ''; if (key) { if (map[key]) map[key].emails = e.count || 0; else map[key] = { date: key, whatsapp: 0, emails: e.count || 0 }; } });
  const result = Object.values(map).sort((a, b) => a.date.localeCompare(b.date));
  if (result.length === 0) {
    const days = [];
    for (let i = 29; i >= 0; i--) { const d = new Date(); d.setDate(d.getDate() - i); days.push({ date: d.toISOString().split('T')[0], whatsapp: 0, emails: 0 }); }
    return days;
  }
  return result;
}
...
export default function PerformanceAreaChart({ data }: { data: { whatsapp: any[]; emails: any[] } }) {
  ...
  return (
    <Card className="bg-black/40 backdrop-blur-xl border-white/10">
      <CardHeader><CardTitle className="text-lg">📈 Email vs WhatsApp (30 days)</CardTitle></CardHeader>
      <CardContent className="h-80">
        {!hasData && <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none"><p className="text-gray-500 text-sm">No activity yet. Send outreach to see data.</p></div>}
