...
import { 
  Download, Copy, FileSpreadsheet, FileJson, FileText, 
  Mail, Phone, ClipboardList, CheckCircle2
} from "lucide-react";
...
export default function ExportMenu({ data, filename = "leads" }: ExportMenuProps) {
  ...
  const copyAll = () => {
    const all = data.map(l => `${l.name} | ${l.company} | ${l.email} | ${l.phone}`).join("\n");
    if (!all) return toast({ title: "No data", variant: "destructive" });
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="sm" className="border-white/20 gap-2">
      ...
      <DropdownMenuContent className="w-56 bg-[#111] border-white/10 text-white">
        ...
        <DropdownMenuItem onClick={copyEmails} className="cursor-pointer hover:bg-white/5 gap-2">
          {copying === "emails" ? <CheckCircle2 className="h-4 w-4 text-green-400" /> : <Mail className="h-4 w-4 text-blue-400" />}
          Copy All Emails
        </DropdownMenuItem>
        
        <DropdownMenuItem onClick={copyPhones} className="cursor-pointer hover:bg-white/5 gap-2">
          {copying === "phones" ? <CheckCircle2 className="h-4 w-4 text-green-400" /> : <Phone className="h-4 w-4 text-green-400" />}
          Copy All Phones
        </DropdownMenuItem>
        
        <DropdownMenuItem onClick={copyAll} className="cursor-pointer hover:bg-white/5 gap-2">
          {copying === "all" ? <CheckCircle2 className="h-4 w-4 text-green-400" /> : <ClipboardList className="h-4 w-4 text-yellow-400" />}
          Copy Complete Data
        </DropdownMenuItem>
